...
public class SerpApiSearchProvider implements SearchProvider {
    ...
    public String unavailableHint() {
        return "Web 搜索未配置 SerpAPI Key。请设置环境变量 SERPAPI_KEY，"
                + "或 SEARCH_PROVIDER=searxng 并配置 SEARXNG_URL 使用自托管 SearXNG。"
                + "申请 Key：https://serpapi.com/manage-api-key";
    }
    ...
    private List<SearchResult> parse(String json, int maxResults) throws IOException {
        ...
        if (results.isEmpty()) {
            ...
            if (!answer.isBlank()) {
                results.add(SearchResult.of(1, "精选摘要", "", answer));
            }
        }
        return results;
    }
    ...
    private String extractAnswerBox(JsonNode answerBox) {
        ...
        return answerBox.path("answer").asText("");
    }
}

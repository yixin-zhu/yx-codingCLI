...
public class StdioTransport implements McpTransport {
    ...
    public StdioTransport(String command, List<String> args, Map<String, String> env, Path workingDir) throws IOException {
        List<String> commandLine = WindowsProcessCommand.buildCommandLine(command, args);
        ProcessBuilder builder = new ProcessBuilder(commandLine);
        if (workingDir != null) {
            builder.directory(workingDir.toFile());
        }
        WindowsProcessCommand.ensureNodeOnPath(builder);
    ...
    private void startStdoutReader() {
        Thread thread = new Thread(() -> {
            try (BufferedReader reader = new BufferedReader(
                ...
                appendStderr("[agent] stdout reader stopped: " + e.getMessage());
            }
        }, "agent-mcp-stdio-stdout");
    ...
    private void startStderrReader() {
        Thread thread = new Thread(() -> {
            try (BufferedReader reader = new BufferedReader(
                ...
                appendStderr("[agent] stderr reader stopped: " + e.getMessage());
            }
        }, "agent-mcp-stdio-stderr");
        thread.setDaemon(true);
        thread.start();
    }
